...
select * from funcionario inner join departamento on departamento.iddepartamento = funcionario.iddepartamento;
select * from funcionario inner join funcao on funcionario.idfuncao = funcao.idfuncao;
SELECT * FROM DEPARTAMENTO INNER JOIN FUNCIONARIO ON DEPARTAMENTO.IDDEPARTAMENTO = FUNCIONARIO.IDDEPARTAMENTO INNER JOIN FUNCAO ON FUNCAO.IDFUNCAO = FUNCIONARIO.IDFUNCAO;
SELECT 	FUNCIONARIO.NOME, DEPARTAMENTO.NOME FROM FUNCIONARIO INNER JOIN DEPARTAMENTO ON FUNCIONARIO.IDDEPARTAMENTO = DEPARTAMENTO.IDDEPARTAMENTO WHERE DEPARTAMENTO.NOME = 'FINANCEIRO';
select max(salario), min(salario), sexo from funcionario group by funcionario.sexo;
select sum(funcionario.salario), avg(funcionario.salario), departamento.nome, departamento.iddepartamento from funcionario inner join departamento on funcionario.iddepartamento = departamento.iddepartamento group by departamento.nome, iddepartamento;
select count(funcionario.idfuncionario), departamento.nome from departamento inner join funcionario on departamento.iddepartamento = funcionario.iddepartamento group by departamento.nome order by departamento.nome;
select sum(funcionario.salario), min(funcionario.salario), max(funcionario.salario), funcao.nome from funcao inner join funcionario on funcao.idfuncao = funcionario.idfuncao group by funcao.nome, funcao.id;
select funcionario.idfuncionario, funcionario.nome, salario, departamento.nome, funcao.nome from funcionario inner join departamento on departamento.iddepartamento = funcionario.iddepartamento inner join funcao on funcionario.idfuncao = funcao.idfuncao;
