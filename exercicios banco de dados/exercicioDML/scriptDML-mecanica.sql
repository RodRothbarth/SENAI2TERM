...
-- 1
select proprietario.nome, proprietario.telefone_residencial, proprietario.telefone_comercial, proprietario.telefone_recado from proprietario order by proprietario.nome;

-- 2
select servico.nome from servico order by servico.nome;

-- 3
select produto.nome from produto order by produto.nome;

-- 4
select veiculo.marca, count(veiculo.idveiculo) from veiculo group by veiculo.marca;

-- 5 date_add(dt_nascimento, interval year(now()) - year(dt_nascimento) year)
select proprietario.nome, proprietario.dt_nascimento from proprietario where (concat_ws('-',year(now()),month(proprietario.dt_nascimento),day(proprietario.dt_nascimento))) >= curdate() and (concat_ws('-',year(now()),month(proprietario.dt_nascimento),day(proprietario.dt_nascimento))) <= date_add(curdate(), interval 45 day) order by proprietario.dt_nascimento;

-- 6
select proprietario.nome, veiculo.modelo, veiculo.placa, veiculo.modelo from proprietario left join veiculo on proprietario.idproprietario = veiculo.idproprietario;

-- 7
 select proprietario.nome, veiculo.modelo, veiculo.placa, veiculo.modelo from proprietario right join veiculo on proprietario.idproprietario = veiculo.idproprietario order by proprietario.nome;
 
 -- 8
 select mecanico.nome from mecanico;
 
 -- 9
 select distinct produto.nome, produto.valor from produto join item_produto on produto.idproduto = item_produto.idproduto join orcamento on orcamento.idorcamento = item_produto.idorcamento;
 
 -- 10
 select veiculo.marca, veiculo.placa, veiculo.modelo, orcamento.idorcamento, orcamento.dt_orcamento, sum(item_servico.qtde), sum(item_servico.qtde * servico.valor) from veiculo 
 join orcamento on orcamento.idveiculo = veiculo.idveiculo join item_servico on item_servico.idorcamento = orcamento.idorcamento join servico on servico.idservico = item_servico.idservico where veiculo.idveiculo = 1;
 
 
 -- 11
 select veiculo.marca, veiculo.placa, veiculo.modelo, orcamento.idorcamento, orcamento.dt_orcamento, sum(item_produto.qtde), sum(item_produto.qtde * produto.valor) from veiculo 
 join orcamento on orcamento.idveiculo = veiculo.idveiculo join item_produto on item_produto.idorcamento = orcamento.idorcamento join produto on produto.idproduto = item_produto.idproduto where veiculo.idveiculo = 1;
 
 -- 12 
 select veiculo.marca, veiculo.placa, veiculo.modelo, orcamento.idorcamento, orcamento.dt_orcamento, mecanico.nome from veiculo join orcamento on orcamento.idveiculo = veiculo.idveiculo join mecanico on mecanico.idmecanico = orcamento.idmecanico where veiculo.idveiculo = 2;
 
 -- 13
 select veiculo.marca, veiculo.modelo, veiculo.placa, veiculo.ano_fabricacao, count(orcamento.idorcamento) from veiculo join orcamento on veiculo.idveiculo = orcamento.idveiculo where veiculo.ano_fabricacao = 2014 group by veiculo.marca, veiculo.modelo, veiculo.placa, veiculo.ano_fabricacao;
